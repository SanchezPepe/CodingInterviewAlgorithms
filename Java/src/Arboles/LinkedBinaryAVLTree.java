...
public class LinkedBinaryAVLTree<T extends Comparable<T>> extends LinkedBinarySearchTree<T> {
    ...
    public LinkedBinaryAVLTree(T elem) {
        super(elem);
        super.raiz = new NodeAVL<T>(elem);
        this.raiz = (NodeAVL<T>) super.raiz;
    }
    ...
    public void rota(NodeAVL<T> nodo) {
        NodeAVL<T> alfa, beta, gamma, A, B, C, D, papa;
    ...
    public void inserta(T elem) {
        NodeAVL<T> actual, papa, nodo = new NodeAVL<T>(elem);
        actual = this.raiz;
        papa = actual;
        cont++;
        if (actual == null) {
        ...
        while (actual != null) {
            ...
            if (elem.compareTo(actual.getElem()) <= 0)
                actual = (NodeAVL<T>) actual.getIzq();
            else
                actual = (NodeAVL<T>) actual.getDer();
        }
        papa.cuelga(nodo);
    ...
    public void imprimeArbol() {
        ...
        Queue<NodeAVL<T>> nivelActual = new LinkedList<NodeAVL<T>>();
        Queue<NodeAVL<T>> nivelSiguiente = new LinkedList<NodeAVL<T>>();
        int contImp = 0;
        nivelActual.add(this.raiz);
        while (!nivelActual.isEmpty() && contImp < cont) {
            Iterator<NodeAVL<T>> iter = nivelActual.iterator();
            while (iter.hasNext() && contImp < cont) {
                NodeAVL<T> nodoActual = iter.next();
            ...
            nivelSiguiente = new LinkedList<NodeAVL<T>>();
        }
